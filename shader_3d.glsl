#pragma language glsl3

#ifdef VERTEX
vec4 position(mat4 transform_projection, vec4 vertex_position) {
    return transform_projection * vertex_position;
}
#endif

#ifdef PIXEL
precision highp float;

uniform float iTime;
uniform vec2 iResolution;
uniform vec4 rotQ;
uniform float cameraDist;
uniform float orbitalScale;
uniform int quantumN;
uniform int quantumL;
uniform int quantumM;
uniform float maxDensity;
uniform vec3 lightDir;
uniform int lightMode;
uniform float radialNorm;
uniform float shNorm;

#define PI 3.14159265358979

vec3 quatRotate(vec3 v, vec4 q) {
    vec3 t = 2.0 * cross(q.xyz, v);
    return v + q.w * t + cross(q.xyz, t);
}

float laguerre(int k, int alpha, float x) {
    if (k <= 0) return 1.0;
    if (k == 1) return 1.0 + float(alpha) - x;
    float L0 = 1.0;
    float L1 = 1.0 + float(alpha) - x;
    for (int i = 2; i <= k; i++) {
        float fi = float(i);
        float fa = float(alpha);
        float L = ((2.0*fi + fa - 1.0 - x) * L1 - (fi + fa - 1.0) * L0) / fi;
        L0 = L1;
        L1 = L;
    }
    return L1;
}

float radial(int n, int l, float r) {
    float rho = 2.0 * r / float(n);
    return radialNorm * exp(-rho * 0.5) * pow(rho, float(l)) * laguerre(n-l-1, 2*l+1, rho);
}

float assocLegendre(int l, int m, float x) {
    int abs_m = m < 0 ? -m : m;
    if (abs_m > l) return 0.0;
    float somx2 = sqrt(max(0.0, 1.0 - x * x));
    float pmm = 1.0;
    for (int i = 1; i <= abs_m; i++) {
        pmm *= float(2 * i - 1) * somx2;
    }
    if (l == abs_m) return pmm;
    float pmmp1 = x * float(2 * abs_m + 1) * pmm;
    if (l == abs_m + 1) return pmmp1;
    float pl_2 = pmm;
    float pl_1 = pmmp1;
    float pl = 0.0;
    for (int i = abs_m + 2; i <= l; i++) {
        float fi = float(i);
        float fm = float(abs_m);
        pl = ((2.0 * fi - 1.0) * x * pl_1 - (fi + fm - 1.0) * pl_2) / (fi - fm);
        pl_2 = pl_1;
        pl_1 = pl;
    }
    return pl;
}

float sphericalHarmonic(int l, int m, float theta, float phi) {
    int abs_m = m < 0 ? -m : m;
    if (abs_m > l) return 0.0;
    float ct = cos(theta);
    float Plm = assocLegendre(l, abs_m, ct);
    if (Plm == 0.0) return 0.0;
    if (m == 0) return shNorm * Plm;
    if (m > 0) return shNorm * Plm * cos(float(m) * phi);
    return shNorm * Plm * sin(float(abs_m) * phi);
}

vec3 getOrbitalColor(float a) {
    vec3 c1 = vec3(0.0, 0.0, 0.0);
    vec3 c2 = vec3(0.15, 0.02, 0.35);
    vec3 c3 = vec3(0.75, 0.05, 0.45);
    vec3 c4 = vec3(1.0, 0.55, 0.15);
    vec3 c5 = vec3(1.0, 0.95, 0.75);
    if (a < 0.2) return mix(c1, c2, a * 5.0);
    if (a < 0.4) return mix(c2, c3, (a - 0.2) * 5.0);
    if (a < 0.7) return mix(c3, c4, (a - 0.4) * 3.333);
    return mix(c4, c5, (a - 0.7) * 3.333);
}

float getDensity(int n, int l, int m, vec3 p, float S) {
    float r = length(p);
    if (r < 0.0001) {
        // Analytically correct at nucleus: R ~ r^l, so l>0 gives 0 automatically
        float R0 = radial(n, l, 0.0);
        float Y0 = sphericalHarmonic(l, m, 0.0, 0.0);
        return R0*R0 * Y0*Y0;
    }
    float theta = acos(clamp(p.y/r, -1.0, 1.0));
    float phi = atan(p.z, p.x);
    float R = radial(n, l, r/S);
    float Y = sphericalHarmonic(l, m, theta, phi);
    return R*R * Y*Y;
}

vec4 effect(vec4 color, Image tex, vec2 tc, vec2 sc) {
    vec2 uv = (2.0 * sc - iResolution) / iResolution.y;
    float hfov = 1.4;
    vec3 ro = vec3(0.0, 0.0, cameraDist);
    vec3 rd = normalize(vec3(uv.x * tan(hfov/2.0), uv.y * tan(hfov/2.0), -1.0));
    ro = quatRotate(ro, rotQ);
    rd = quatRotate(rd, rotQ);

    int n = quantumN;
    int l = quantumL;
    int m = quantumM;
    float S = orbitalScale;
    float maxR = (float(n) * float(n) * 2.0) * S;

    float b = dot(ro, rd);
    float c = dot(ro, ro) - maxR * maxR;
    float h = b * b - c;
    float tStart = 0.0, tEnd = 0.0;
    if (h >= 0.0) {
        float sqrtH = sqrt(h);
        tStart = max(0.0, -b - sqrtH);
        tEnd = -b + sqrtH;
    }
    if (tEnd <= tStart || tEnd <= 0.0) return vec4(vec3(0.005, 0.005, 0.01), 1.0);

    float eps = 0.02 * S;
    float alpha = 0.0;
    vec3 col = vec3(0.0);
    const int steps = 80;
    float t = tStart;
    float totalDist = tEnd - tStart;

    // Dither start to reduce banding
    t += (totalDist / float(steps)) * fract(sin(dot(sc, vec2(12.9898, 78.233))) * 43758.5453);

    for (int i = 0; i < steps; i++) {
        if (alpha > 0.98) break;
        vec3 p = ro + rd * t;
        float r = length(p);

        // Adaptive step: larger steps further out
        float dt = (totalDist / float(steps)) * (0.6 + 1.4 * smoothstep(0.0, maxR, r));

        float density = getDensity(n, l, m, p, S);
        if (density > 1e-14) {
            float distFade = smoothstep(maxR, maxR * 0.9, r);
            float a = clamp((density * distFade) / maxDensity, 0.0, 1.0);

            // Non-linear opacity: makes low-density regions much more transparent
            // while keeping high-density regions visible.
            float weight = pow(a, 1.4);
            float opacity = 1.0 - exp(-weight * 8.0 * dt / S);

            if (opacity > 0.001) {
                vec3 rgb = getOrbitalColor(a);

                if (lightMode == 1 && a > 0.15) {
                    float dx = getDensity(n, l, m, p + vec3(eps, 0, 0), S) - density;
                    float dy = getDensity(n, l, m, p + vec3(0, eps, 0), S) - density;
                    float dz = getDensity(n, l, m, p + vec3(0, 0, eps), S) - density;
                    vec3 normal = normalize(vec3(dx, dy, dz));

                    float diffuse = max(0.0, dot(normal, lightDir));
                    float rim = pow(1.0 - max(0.0, dot(normal, -rd)), 3.0);
                    rgb *= 0.4 + 0.6 * diffuse + 0.3 * rim;

                    // Add a slight emissive boost for high density
                    rgb += pow(a, 3.0) * vec3(0.2, 0.1, 0.0);
                } else {
                    rgb *= 1.1 + a * 0.5; // Brighter core even without light
                }

                // Blend with density weighting
                float sa = opacity * (1.0 - alpha);
                col += rgb * sa;
                alpha += sa * (0.5 + 0.5 * a); // High density accumulates alpha faster than low density
            }
        }
        t += dt;
        if (t > tEnd) break;
    }

    float nucGlow = exp(-alpha * 3.0) * 0.15 * (0.8 + 0.2 * sin(iTime * 1.5));
    col += vec3(0.4, 0.2, 1.0) * nucGlow;
    return vec4(mix(vec3(0.005, 0.005, 0.01), col, min(alpha * 1.2, 1.0)), 1.0);
}
#endif
